...
load_dotenv(r"C:\Users\admin\autosense-ai\backend\.env")
...
def predict(req: PredictRequest):
    ...
    xgb_pred = float(xgb_model.predict(features_array)[0])
    lgbm_pred = float(lgbm_model.predict(features_array)[0])
    predicted_price = round((xgb_pred + lgbm_pred) / 2, 0)
